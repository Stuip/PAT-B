 ...
 * 我们只要在调用 f 之前先调用 clock()，获得一个时钟打点数 C1；在 f 执行完成后再调用 clock()，获得另一个时钟打点数 C2；
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int clock = ((C2-C1) + 50)/CLK_TCK;
        int hour = clock/3600;
        int minute = (clock - hour*60*60)/60;
        int second = Math.round(clock - hour*60*60 - minute*60);
